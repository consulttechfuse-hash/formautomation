...
export default function UnlockRequest() {
  ...
  const loadClients = async () => {
    ...
    // Get clients who have completed Form-01 (onboarding_complete = true)
    const { data: clientsData } = await supabase
      .from('user_roles')
      .select('user_id, email, first_name, last_name, onboarding_complete')
      .eq('role', 'client')
  ...
  return (
    <div className="space-y-8">
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <p className="text-sm text-gray-600 mb-4">Request unlocking of Form-01 for a client who needs to make corrections.</p>
        ...
        <div className="space-y-4">
          <div>
            ...
            <select value={selectedClient} onChange={(e) => setSelectedClient(e.target.value)} className="w-full border rounded-lg px-3 py-2" disabled={submitting}>
              <option value="">-- Select a client --</option>
              {clients.map((client) => (
                <option key={client.id} value={client.id}>{client.first_name} {client.last_name} ({client.email})</option>
              ))}
            </select>
            {clients.length === 0 && <p className="text-sm text-gray-500 mt-1">No clients with completed Form-01 found.</p>}
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Reason for Unlock</label>
            <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={3} className="w-full border rounded-lg px-3 py-2" placeholder="Explain why the client needs to edit Form-01..." disabled={submitting} />
          </div>

          <button onClick={handleSubmit} disabled={submitting || !selectedClient || !reason.trim() || clients.length === 0} className="w-full bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 disabled:opacity-50">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        {pendingRequests.length === 0 ? (
          <p className="text-gray-500 text-center py-4">No unlock requests submitted.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr><th className="px-4 py-2 text-left text-sm font-medium text-gray-500">Client</th><th className="px-4 py-2 text-left text-sm font-medium text-gray-500">Reason</th><th className="px-4 py-2 text-left text-sm font-medium text-gray-500">Requested</th><th className="px-4 py-2 text-left text-sm font-medium text-gray-500">Status</th></tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {pendingRequests.map((req) => (
                  <tr key={req.id} className="hover:bg-gray-50">
                    <td className="px-4 py-2"><div className="font-medium">{req.client_name}</div><div className="text-xs text-gray-500">{req.client_email}</div></td>
