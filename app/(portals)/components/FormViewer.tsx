...
export default function FormViewer({ formData, formNumber, clientEmail, showEditButton = false, showAddField = false }: FormViewerProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [editData, setEditData] = useState<any>(formData);
  ...
  const handleFieldAdded = (newField: { fieldName: string; fieldLabel: string; fieldType: string }) => {
    loadCustomFields();
    setEditData({ ...editData, [newField.fieldName]: '' });
  };

  const renderField = (label: string, fieldName: string, value: any, type: string = 'TEXT', keyIndex: number) => {
    if (isEditing) {
      if (type === 'TEXTAREA') {
        return (
      ...
      <div className="space-y-6">
        <div className="bg-blue-50 p-4 rounded sticky top-0 flex justify-between items-center flex-wrap gap-2">
          ...
          <div className="flex gap-2">
            ...
            {showAddField && (
              <button onClick={() => setShowAddFieldModal(true)} className="bg-purple-600 text-white px-4 py-2 rounded hover:bg-purple-700">
                ➕ Add Field
              </button>
            )}
            {isEditing && (
        ...
        <div className="space-y-4">
          <div className="border rounded-lg p-4 bg-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {standardFields.slice(0, 5).map((field, idx) => renderField(field.label, field.name, editData[field.name], field.type, idx))}
            </div>
          </div>
          ...
          <div className="border rounded-lg p-4 bg-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {standardFields.slice(5, 7).map((field, idx) => renderField(field.label, field.name, editData[field.name], field.type, idx + 10))}
            </div>
          </div>
          ...
          {customFields.length > 0 && (
            <div className="border rounded-lg p-4 bg-white">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {customFields.map((field, idx) => renderField(field.field_label, field.field_name, editData[field.field_name], field.field_type === 'TEXTAREA' ? 'TEXTAREA' : 'TEXT', idx + 100))}
              </div>
            </div>
          )}
        </div>

        {showAddFieldModal && <AddFieldModal onClose={() => setShowAddFieldModal(false)} onFieldAdded={handleFieldAdded} />}
      </div>
    );
  }
  ...
  return (
    <div className="bg-yellow-50 p-4 rounded">
      ...
      <p className="text-xs text-gray-500 mt-2">The client has not completed this form yet.</p>
    </div>
  );
}
